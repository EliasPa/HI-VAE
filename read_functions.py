...
def read_data(data_file, types_file, miss_file, true_miss_file):
    ...
    if true_miss_file:
        with open(true_miss_file, 'r') as f:
            missing_positions = [[int(x) for x in rec]
                                 for rec in csv.reader(f, delimiter=',')]
            missing_positions = np.array(missing_positions)

        true_miss_mask = np.ones([np.shape(data)[0], len(types_dict)])
        # Indexes in the csv start at 1
        true_miss_mask[missing_positions[:, 0] -
                       1, missing_positions[:, 1]-1] = 0
    ...
    for i, batch in enumerate(samples):
        if i == 0:
            ...
            samples_x = np.concatenate(
                [samples_x, np.concatenate(batch['x'], 1)], 0)
    ...
    for d in range(len(types_dict)):
        ...
        if types_dict[d]['type'] == 'cat':
            output.append(np.reshape(
                np.argmax(data[:, ind_ini:ind_end], 1), [-1, 1]))
        elif types_dict[d]['type'] == 'ordinal':
            output.append(np.reshape(
                np.sum(data[:, ind_ini:ind_end], 1) - 1, [-1, 1]))
        else:
...
def mean_imputation(train_data, miss_mask, types_dict):
    ...
    for dd in range(len(types_dict)):
        ...
        if types_dict[dd]['type'] == 'cat' or types_dict[dd]['type'] == 'ordinal':
            ...
            # print(train_data[:, ind_ini:ind_end])
            # print(np.unique(train_data[:, ind_ini:ind_end]))
            # print(train_data[miss_pattern,ind_ini:ind_end])
            values, counts = np.unique(
                train_data[miss_pattern, ind_ini:ind_end], return_counts=True)
            if (len(values) == 0):
                col_data = train_data[:, ind_ini:ind_end]
        ...
        # Imputation for the rest of the variables is done with the mean of the data
            ...
            data_imputed = train_data[:, ind_ini:ind_end]*miss_mask[:,
                                                                    ind_ini:ind_end] + data_mean*(1.0-miss_mask[:, ind_ini:ind_end])
                ...
                if i == 0:
                    ...
                    for v in range(len(types_dict)):
                        if types_dict[v]['type'] == 'pos' or types_dict[v]['type'] == 'real':
                            out_dict[k][v] = np.concatenate(
                                [out_dict[k][v], batch[k][v]], 1)
                        else:
                            out_dict[k][v] = np.concatenate(
                                [out_dict[k][v], batch[k][v]], 0)
...
def statistics(loglik_params, types_dict):
    ...
    for d, attrib in enumerate(loglik_params):
        ...
        # Only for log-normal
            ...
            loglik_mean.append(np.maximum(
                np.exp(attrib[0] + 0.5*attrib[1]) - 1.0, 0.0))
            loglik_mode.append(np.maximum(
                np.exp(attrib[0] - attrib[1]) - 1.0, 0.0))
    ...
    for dd in range(len(types_dict)):
        ...
        # Mean "shift" error
            ...
            error_observed.append(np.mean(np.abs(
                x_train[miss_mask[:, dd] == 1, ind_ini:ind_end] - x_hat[miss_mask[:, dd] == 1, ind_ini:ind_end]))/int(types_dict[dd]['dim']))
            if np.sum(miss_mask[:, dd] == 0, 0) == 0:
                error_missing.append(0)
            else:
                error_missing.append(np.mean(np.abs(
                    x_train[miss_mask[:, dd] == 0, ind_ini:ind_end] - x_hat[miss_mask[:, dd] == 0, ind_ini:ind_end]))/int(types_dict[dd]['dim']))
        # Normalized root mean square error
            ...
            if(len(a) == 0 or len(b) == 0):
                ...
                error_observed.append(
                    np.sqrt(mean_squared_error(a, b))/norm_term)

            if np.sum(miss_mask[:, dd] == 0, 0) == 0 or len(a) == 0 or len(b) == 0:
                error_missing.append(0)
            else:
                error_missing.append(np.sqrt(mean_squared_error(
                    x_train[miss_mask[:, dd] == 0, ind_ini: ind_end], x_hat[miss_mask[:, dd] == 0, ind_ini: ind_end]))/norm_term)
